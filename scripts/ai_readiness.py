...
Checks AI crawler access, structured data depth, content structure, content ratio, semantic HTML.

Note: llms.txt and ai.txt are deliberately excluded. No major AI company (Google, OpenAI,
Anthropic, Meta) confirms reading them. Including them would inflate scores without improving
actual AI visibility. See citability.dev/methodology for rationale.
"""
...
def check_ai_crawler_access(url: str) -> dict:
    """Check 2.1: AI Crawler Access Directives."""
    parsed = urlparse(url)
    base = f"{parsed.scheme}://{parsed.netloc}"
    result = {
        "check": "2.1_ai_crawler_access",
        "tier": "VERIFIABLE",
        "crawlers": {},
        "verdict": "FAIL",
    }
...
def _vertical_rich_types(vertical: str | None) -> set[str]:
    """Pick the 'rich' schema-type set for the chosen vertical.

    Returns the publisher-biased default set when no vertical is provided
    (preserves prior behavior). When a vertical is provided, returns that
    vertical's schema_types from verticals.py so e.g. local-healthcare
    weights LocalBusiness/MedicalBusiness/Dentist instead of FAQPage/Article.
    """
    default_rich = {"FAQPage", "HowTo", "Article", "BlogPosting", "Recipe", "Product", "Review"}
    if not vertical:
        return default_rich
    try:
        from verticals import get_vertical
        return set(get_vertical(vertical).schema_types)
    except Exception:
        # Unknown vertical or import error — fall back to publisher default
        return default_rich


def check_structured_data_depth(url: str, vertical: str | None = None) -> dict:
    """Check 2.2: Structured Data Depth.

    When `vertical` is set, the 'rich types' check uses that vertical's
    weighted schema_types from verticals.py (e.g. local-healthcare weights
    LocalBusiness/MedicalBusiness/Dentist). When vertical is None, falls back
    to the publisher-biased default set for backwards compatibility.
    """
    parsed = urlparse(url)
    base = f"{parsed.scheme}://{parsed.netloc}"
    result = {
    ...
    rich_types = _vertical_rich_types(vertical)
    has_rich_types = bool(schema_types_found & rich_types)
    matched_rich = sorted(schema_types_found & rich_types)
    ...
    result["details"] = {
        ...
        "vertical": vertical,
        "rich_types_expected": sorted(rich_types),
        "rich_types_matched": matched_rich,
    }

    # Vertical-aware verdict: if a vertical is set, MISSING the vertical's
    # rich types is a stronger signal than schema-type count alone. A local
    # ortho without LocalBusiness/MedicalBusiness fails the 'rich' check
    # even if it has 5 generic Article schemas.
    if vertical and not has_rich_types:
        result["verdict"] = "FAIL"
    elif coverage > 0.8 and len(schema_types_found) >= 3:
        result["verdict"] = "PASS"
...
def check_content_structure(url: str) -> dict:
    """Check 2.3: Content Structure Quality."""
    result = {
        "check": "2.3_content_structure",
        "tier": "VERIFIABLE",
        "details": {},
        "verdict": "FAIL",
    }
...
def check_content_ratio(url: str) -> dict:
    """Check 2.4: Machine-Readable Content Ratio."""
    result = {
...
def check_semantic_html(url: str) -> dict:
    """Check 2.5: Semantic HTML and Accessibility."""
    result = {
        "check": "2.5_semantic_html",
        "tier": "VERIFIABLE",
        "details": {},
        "verdict": "FAIL",
    }
...
def run_ai_readiness(url: str, vertical: str | None = None) -> dict:
    """Run all AI Infrastructure Readiness checks for a URL.

    Args:
        url: target URL
        vertical: optional vertical profile name from verticals.py
            ('local-healthcare', 'saas-tool', 'personal-brand', 'tech-publisher').
            When set, structured-data check weights vertical-appropriate
            schema types (e.g. LocalBusiness/MedicalBusiness for local-healthcare)
            instead of the publisher-biased default.
    """
    results = {
    ...
    results["checks"].append(check_structured_data_depth(url, vertical=vertical))
